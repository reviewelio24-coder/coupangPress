...
async function runJobPipeline(job, publishJobResult) {
  ...
  if (updated?.autoPublish && updated.status === 'completed') {
...
async function tick(publishJobResult) {
  ...
  processing = true;
  try {
    await runJobPipeline(job, publishJobResult);
  } catch (err) {
...
function startWorker(publishJobResult) {
  ...
  setInterval(() => tick(publishJobResult), 1500);
  tick(publishJobResult);
}
