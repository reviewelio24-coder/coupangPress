...
const META_DESC_MAX = 156;

function clipMetaDescription(text) {
  let s = String(text || '')
    .replace(/\s+/g, ' ')
    .replace(/^["'\s]+|["'\s]+$/g, '')
    .trim();
  if (s.length <= META_DESC_MAX) return s;
  s = s.slice(0, META_DESC_MAX);
  const punct = Math.max(s.lastIndexOf('.'), s.lastIndexOf('다 '), s.lastIndexOf('요 '));
  if (punct >= 80) return s.slice(0, punct + 1).trim();
  const space = s.lastIndexOf(' ');
  if (space >= 80) return s.slice(0, space).trim();
  return s.trim();
}

function buildMetaDescriptionTemplate(payload, title) {
  const products = normalizeProducts(payload);
  const kw = normalizeKeyword(payload.seoKeyword);
  const first = String(products[0]?.productTitle || '').trim().slice(0, 36);
  const n = products.length;
  const who = n > 1 ? `${n}개 상품` : first;
  const core = kw
    ? `${kw} 추천: ${who ? `${who} ` : ''}실제 구매 후기를 모아 장단점, 사용 팁, 구매 전 체크 포인트를 정리했습니다.`
    : `${title || who || '상품'} 실제 구매 후기를 바탕으로 장단점과 선택 포인트를 정리했습니다.`;
  return clipMetaDescription(core);
}

async function buildMetaDescription(payload, post, config) {
  const fallback = buildMetaDescriptionTemplate(payload, post.title);
  const kw = normalizeKeyword(payload.seoKeyword);
      ...
      [
        {
          role: 'system',
          content:
            '한국어 SEO 메타 설명 작성기. Yoast용 검색 스니펫 문장만 JSON으로 반환.'
        },
        {
          role: 'user',
          content: `글 제목: ${post.title}
초점 키프레이즈: 「${kw}」
상품: ${normalizeProducts(payload)
            .map((p) => p.productTitle)
            .join(' / ')}

규칙:
- 120~156자 한국어 한 문단
- 초점 키프레이즈를 1회 자연스럽게 포함
- 구매 후기 기반 추천글임을 알릴 것
- 따옴표·해시태그·이모지 금지
- JSON만: {"metaDescription":"..."}`
        }
      ],
      config
    );
    const desc = clipMetaDescription(parsed.metaDescription);
    if (desc.length >= 50) {
      if (desc.includes(kw)) return desc;
      return clipMetaDescription(`${kw} ${desc}`);
    }
  } catch (err) {
    console.warn('메타 설명 OpenAI 생성 실패, 기본 문구 사용:', err.message);
  }
  return fallback;
}

function withSeoFields(post, payload, metaDescription) {
  return {
    ...post,
    focusKeyphrase: normalizeKeyword(payload.seoKeyword),
...
async function generatePost(payload, config) {
  ...
  if (config.openaiApiKey) {
    try {
      const post = await buildWithOpenAI(base, config);
      const metaDescription = await buildMetaDescription(base, post, config);
      return withSeoFields(post, base, metaDescription);
    } catch (err) {
      console.warn('OpenAI 생성 실패, 템플릿으로 대체:', err.message);
    }
  }

  const post = buildTemplatePost(base);
  return withSeoFields(post, base, buildMetaDescriptionTemplate(base, post.title));
}
